...
class PodChallenge():
    ...
    def evaluate_model(self, y_pred, y_true, num_predictors, model_name='Model'):
        '''

        '''
        print(f'{model_name}')
        print(f'R_Squared:          {r2_score(y_true, y_pred)}')
        print(f'Adjusted R_Squared: {1-(1-r2_score(y_true, y_pred))*((len(y_true)-1)/(len(y_true)-num_predictors-1))}')
    ...
    def model_tf(self, quantiles, batch_size):
        '''This is a tensorflow standard ANN that uses linear regression to predict
        quantile regression results. The loss function used in the PinBall loss
        function.

        Args:
            quantiles(list): a list of quantiles to be used for Prediction
            batch_size(int): batch size to be used in the model
        ...
        # Create data matrices, these can be adjusted accordingly. Just make
        # the value to be predicted is in the last column.
        self.df_train_dm = self.df_train[['season', 'month', 'day_of_week', 'k_index', 'temp_mean1256', 'solar_mean123456', 'demand_MW']]
        self.df_train_pv = self.df_train[['season', 'month', 'day_of_week', 'k_index', 'temp_mean1256', 'solar_mean123456', 'pv_power_mw']]
        ...
        # Split data, with the final week being the test data. This is designed to
        # mimick the actual task week implementation. It may be prudent to add
        # a cross-validation split, as well.
        self.X_dm_train, self.X_dm_test, self.y_dm_train, self.y_dm_test = train_test_split(X_dm, y_dm,
                                                                             test_size=336,
                                                                             shuffle=False)
        self.X_pv_train, self.X_pv_test, self.y_pv_train, self.y_pv_test = train_test_split(X_pv, y_pv,
        ...
        # Scale the data, all NNs need scaled data for best # performance.
        sc = StandardScaler()
        self.X_dm_train = sc.fit_transform(self.X_dm_train)
        self.X_dm_test = sc.transform(self.X_dm_test)
        self.X_pv_train = sc.fit_transform(self.X_pv_train)
        self.X_pv_test = sc.transform(self.X_pv_test)

        def tilted_loss(q, y, f):
            '''Calculates loss function for quantile regression NN

            Args:
                q(float): the quantile
                y(float): the actual value of y
                f(float): the predicted value of y
            Returns:
                The calculated loss.
            '''
            error = (y-f)
        ...
        def build_model():
            '''Build the model using keras. This needs to be hypertuned and played
            with.

            Returns:
                The model sturcture to be used in prediction.
            '''
        ...
        # This for loop will make predictions using the build model for each
        # quantile passed into the method. It then appends each list of predictions
        # to an output array.
        for q in self.quantiles:
            model = build_model()
            model.compile(loss=tfa.losses.PinballLoss(tau=q),
                          optimizer='adam')
            self.history = model.fit(self.X_dm_train, self.y_dm_train,
                      epochs=100,
            ...
            self.y_dm_pred = model.predict(self.X_dm_test)
            self.outputs.append(self.y_dm_pred) # Output array with each quantile prediction.
